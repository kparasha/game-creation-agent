...
Most edits in a Reference-style editor are **parameter tweaks on an existing template** — they don't
...
> (Reference' "Effects Mode" pattern). The **Editor Control Manifest is hierarchical** (group → tab →
> control → sub-control) so we can deep-crawl Reference' menu depth later without reworking the shell.
...
- **Privacy + latency**: on-device tweaks are instant and private (no round-trip), which makes the
  iterate loop feel native — exactly where Reference' direct-manipulation shines.
